               ...
               mbDeactivateLocalizationMode(false)
{
    ...
    //std::thread* defe  = new thread(&ORB_SLAM2::LocalMapping::test);
...
cv::Mat System::GetCurrFramePose(){
   ...
   if(mpTracker->mState == 2 && !CurrentFramePose.empty()){
     cv::Mat Rwc = CurrentFramePose.rowRange(0,3).colRange(0,3).t();
     cv::Mat twc = -Rwc*CurrentFramePose.rowRange(0,3).col(3);
     Rwc.copyTo(CurrentFramePose.rowRange(0,3).colRange(0,3));
     twc.copyTo(CurrentFramePose.rowRange(0,3).col(3));
     /*
     cout << Rwc.at<float>(0,0) << " " << Rwc.at<float>(0,1)  << " " << Rwc.at<float>(0,2) << " "  << twc.at<float>(0) << " " <<
